  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white px-6 py-12">
      <div className="max-w-4xl mx-auto">
        {/* Blog Title */}
        <h1 className="text-4xl font-extrabold mb-6 text-indigo-400">{blog.title}</h1>
        ...
        {blog.image && (
          <img
            src={blog.image}
            alt={blog.title}
            className="mb-8 rounded-2xl object-cover w-full max-h-[400px] shadow-lg"
          />
        )}
        ...
        {/* Comments Section */}
        <div className="mt-12 bg-gray-800 p-6 rounded-2xl shadow-xl">
              ...
              comments.map((c, idx) => (
                <div key={idx} className="bg-gray-900 p-4 rounded-lg border border-gray-700">
                  <p className="text-gray-200 text-sm">
                    <span className="font-semibold text-indigo-400">User {idx + 1}:</span>{" "}
                    {c}
                  </p>
                </div>
              ))
